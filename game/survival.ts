...
export const freshStats=():Stats=>({health:100,water:82,energy:88,safety:70,knowledge:0,shelter:0});
export const clamp=(n:number)=>Math.max(0,Math.min(100,n));
export function applyChanges(stats:Stats,changes:Partial<Stats>):Stats{return Object.fromEntries(Object.entries(stats).map(([k,v])=>[k,clamp(v+(changes[k as keyof Stats]||0))])) as unknown as Stats;}
